...
async def add_booking(booking_data: SBookingAdd, db: DBDep):
    return await BookingService(db).add_booking(booking_data)

@router.get("/debug/user/{user_id}")
async def debug_user_bookings(user_id: int, db: DBDep):
