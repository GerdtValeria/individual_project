...
async def edit_comment(rent_id: int, id:int, comment_data: SCommentAdd,db: DBDep,) -> dict[str, str]:
    await CommentService(db).edit_comment(id,comment_data)
    return {"message": "Comment updated successfully"}

@router.delete("/{rent_id}/comments/{id}")
async def delete_comment(rent_id: int, id:int, db: DBDep,) -> dict[str, str]:
